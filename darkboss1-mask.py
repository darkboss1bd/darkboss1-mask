...
import mmap
import tempfile
import webbrowser  # Added to auto-open website
...
def print_hacking_background():
    """ Print a stylish hacking-themed ASCII art background """
    hacking_art = r"""
    ================================================================================
    █▀▀ █▀█ █▀▄▀█ █▀█ █░█ █▀▀ █▀█ █▀▄▀█ █▀▀ █▀█
    ██▄ █▄█ █░▀░█ █▄█ █▄█ ██▄ █▄█ █░▀░█ ██▄ █▀▄
    ...
    ░▒▓█▓▒░░▒▓█▓▒░▒▓█▓▒░░▒▓█▓▒░▒▓█▓▒░░▒▓█▓▒░▒▓█▓▒░░▒▓█▓▒░▒▓████████▓▒░ 
    ================================================================================
    Coded by darkboss1bd | Telegram: https://t.me/darkvaiadmin | Website: https://serialkey.top/
    ================================================================================
    """
    print(Fore.GREEN + hacking_art + Style.RESET_ALL)

# Print background when script starts
print_hacking_background()

# Auto-open the user's website in the default browser
webbrowser.open("https://serialkey.top/")

message = r"""  # CHANGED to raw string
$$\   $$\  $$$$$$\   $$$$$$\  $$\   $$\ $$$$$$$$\ $$$$$$$\  
$$ |  $$ |$$  __$$\ $$  __$$\ $$ | $$  |$$  _____|$$  __$$\ 
$$ |  $$ |$$ /  $$ |$$ /  \__|$$ |$$  / $$ |      $$ |  $$ |
$$$$$$$$ |$$$$$$$$ |$$ |      $$$$$  /  $$$$$\    $$$$$$$  |
...
DEBUG = True
# ######################  Censys.io  ######################### #
# ALL CENSYS CODE HAS BEEN REMOVED
#######################################################
def checkFile():
    ...
    try:
        ...
        for answer in answers:
            ...
            if record == 'NS':
                ...
                domain_name = re.search(r' (.*)\.', answer.to_text(), re.IGNORECASE).group(1)
...
def GetHostnames(results, value):
    ...
    res = CleanHTML(res)
    # CHANGED to raw string
    temp = re.compile(r'[a-zA-Z0-9.-]*\.' + value)
...
def GoogleSearch(value, limit, uas, proxies, timeouts):
    quantity = 100
    step = 100
    url = r"https://www.google.com/search?num={quantity}&start={counter}&hl=en&meta=&q=%40%22{value}%22"
    return CommonSearch(value, url, quantity, step, limit, uas, proxies, timeouts)
def BingSearch(value, limit, uas, proxies, timeouts):
    quantity = 50
    step = 50
    url = r"https://www.bing.com/search?q=%40{value}&count={quantity}&first={counter}"
    return CommonSearch(value, url, quantity, step, limit, uas, proxies, timeouts)
def AskSearch(value, limit, uas, proxies, timeouts):
    step = 1
    url = r"https://www.ask.com/web?q=%40%22{value}%22&page={counter}"
    return CommonSearch2(value, url, step, limit, uas, proxies, timeouts)
# todo captcha
def DogpileSearch(value, limit, uas, proxies, timeouts):
    step = 15
    url = r"https://www.dogpile.com/search/web?qsi={counter}&q=%40{value}"
    return CommonSearch2(value, url, step, limit, uas, proxies, timeouts)
def YahooSearch(value, limit, uas, proxies, timeouts):
    step = 10
    url = r"https://search.yahoo.com/search?p=%40{value}&b={counter}&pz=10"
    return CommonSearch2(value, url, step, limit, uas, proxies, timeouts)
...
def DNSDumpsterSearch(targetip, uas, proxies):
    ...
    try:
        ...
        # get csrf token
        # CHANGED to raw string
        csrf_regex = re.compile(r'name="csrfmiddlewaretoken" value="(.*?)"', re.S)
        try:
...
def MainFunc():
    ...
    # The following censys-related arguments are kept for compatibility but will be ignored.
    parser.add_argument('-m', '--match', default=None,
                        help='[IGNORED] Highlight a string within an existing query result')
    parser.add_argument('-f', '--filter', default=None,
                        help="[IGNORED] Filter the JSON keys to display for each result")
    parser.add_argument('--count', action='store_true', help='[IGNORED] Print the count result and exit')
    parser.add_argument('-R', '--report', default=None,
                        help="[IGNORED] Stats on given field")
    parser.add_argument('-B', '--report_bucket', default=report_buckets,
                        help='[IGNORED] Bucket len in report mode (default: %s)' % report_buckets)
    # query filter shortcuts - censys.io (Ignored)
    parser.add_argument('-1', '--censys_api_id', action='store', metavar='CENSYS_API_ID',
                        dest='censys_api_id', type=str, default=None,
                        help='[IGNORED] Provide the authentication ID for the censys.io search engine')
    ...
    parser.add_argument('-H', '--html', action='store_true',
                        help='[IGNORED] Renders html elements in a browser')
    parser.add_argument('arguments', metavar='arguments', nargs='*', help='[IGNORED] Censys query')
    ...
    #######################################################
    # Censys.io search (REMOVED) #
    # This entire block is commented out or removed.
    # if any(i in ['censys'] for i in info['mode']):
    #     ... (All censys code removed)
    #######################################################
